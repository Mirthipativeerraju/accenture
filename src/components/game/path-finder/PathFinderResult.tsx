...
export function PathFinderResult() {
  ...
  currentSession.actions.forEach((a) => {
    if (a.payload && (a.payload as { isTimeout?: boolean }).isTimeout) {
      timeouts++;
    } else if (a.valid) {
      correct++;
    }
    ...
    <div className="flex w-full flex-col items-center justify-center py-8">
      <Card className="w-full max-w-2xl text-center">
        <CardHeader>
          ...
          <p className="mt-2 text-muted-foreground">Path Finder</p>
        </CardHeader>

        <CardContent className="space-y-8">
          <div className="flex flex-col items-center justify-center">
            <span className="text-6xl font-black text-primary">{score}</span>
            <span className="mt-2 font-medium uppercase tracking-wider text-muted-foreground">Score</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <div className="flex flex-col items-center p-4 rounded-xl bg-muted/50">
              <div className="flex h-5 w-5 mb-2 items-center justify-center rounded-full bg-destructive/20 text-destructive font-bold text-[10px]">
                X
              </div>
        ...
        <CardFooter className="flex flex-col sm:flex-row gap-4 justify-center mt-4">
          ...
          <Link href="/practice/path-finder" className="w-full sm:w-auto">
